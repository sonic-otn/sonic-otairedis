        ...
        _In_ const lai_attribute_t *attr_list)
{
    ...
    SWSS_LOG_DEBUG("generic create key: %s, fields: %zu", key.c_str(), entry.size());
        ...
        _In_ const lai_attribute_t *attr_list)
{
    ...
    SWSS_LOG_DEBUG("generic get key: %s, fields: %zu", key.c_str(), entry.size());
        ...
        _In_ const lai_attribute_t *attrList)
{
    ...
    SWSS_LOG_DEBUG("Query arguments: linecard: %s, attributes: %s", key.c_str(), joinFieldValues(values).c_str());
        ...
        _Out_ lai_attr_capability_t *Capability)
{
    ...
    SWSS_LOG_DEBUG("Query arguments: linecard %s, object_type: %s, attribute: %s",
                key.c_str(),
...
void Recorder::recordAlarms(
        _In_ bool enable)
{
    SWSS_LOG_ENTER();

    m_recordAlarms = enable;
}
