...
std::shared_ptr<Lai> redis_lai = std::make_shared<Lai>();
        ...
        _In_ const lai_service_method_table_t* service_method_table)
{
    ...
    return redis_lai->initialize(flags, service_method_table);
}
...
#define API(api) .api ## _api = const_cast<lai_ ## api ## _api_t*>(&redis_ ## api ## _api)

static lai_apis_t redis_apis = {
    API(linecard),
    API(port),
    API(transceiver),
    API(logicalchannel),
...
static_assert((sizeof(lai_apis_t)/sizeof(void*)) == (LAI_API_EXTENSIONS_MAX - 1), "Invalid lai_apis_t size");
        ...
        _Out_ void** api_method_table)
{
    ...
    {
        *api_method_table = ((void**)&redis_apis)[lai_api_id - 1];
        ...
        _Out_ lai_attr_capability_t *capability)
{
    ...
    return redis_lai->queryAttributeCapability(
            linecard_id,
            object_type,
            attr_id,
            capability);
        ...
        _Inout_ lai_s32_list_t *enum_values_capability)
{
    ...
    return redis_lai->queryAattributeEnumValuesCapability(
            linecard_id,
            object_type,
            attr_id,
        ...
        _Out_ uint64_t *count)
{
    ...
    return redis_lai->objectTypeGetAvailability(
            linecard_id,
            object_type,
            attr_count,
            attr_list,
        ...
        _In_ lai_object_id_t objectId)
{
    ...
    return redis_lai->objectTypeQuery(objectId);
}
        ...
        _In_ lai_object_id_t objectId)
{
    ...
    return redis_lai->linecardIdQuery(objectId);
}
