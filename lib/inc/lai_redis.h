...
#define REDIS_GET_ALARMS(OT,ot)                          \
    static lai_status_t redis_get_ ## ot ## _alarms(     \
            _In_ lai_object_id_t object_id,             \
...
#define REDIS_CLEAR_ALARMS(OT,ot)                        \
    static lai_status_t redis_clear_ ## ot ## _alarms(   \
            _In_ lai_object_id_t object_id,             \
            _In_ uint32_t number_of_alarms,           \
            _In_ const lai_alarm_type_t *alarm_ids)      \
{                                                       \
    SWSS_LOG_ENTER();                                   \
    return redis_lai->clearAlarms(                       \
            (lai_object_type_t)LAI_OBJECT_TYPE_ ## OT,  \
            object_id,                                  \
            number_of_alarms,                         \
            alarm_ids);                               \
...
#define REDIS_GENERIC_ALARMS(OT, ot)    \
    REDIS_GET_ALARMS(OT,ot);            \
    REDIS_CLEAR_ALARMS(OT,ot);

// ALARMS API

#define REDIS_GENERIC_ALARMS_API(ot)     \
    redis_get_ ## ot ## _alarms,         \
    redis_clear_ ## ot ## _alarms,
