...
void TimerWatchdog::threadFunction()
{
    ...
    while (m_run)
    {
        ...
        {
            SWSS_LOG_NOTICE(" span < 0 = %" PRId64 "at %" PRId64, span, now);
            ...
            SWSS_LOG_NOTICE(" new span  = %" PRId64, span);

            if (span < 0)
                SWSS_LOG_THROW("negative span 'now - start': %" PRId64 " - %" PRId64, now, start);
            ...
            if (span > m_warnTimespan)
            {
                ...
                SWSS_LOG_WARN("time (span < 0) watchdog exceeded %" PRId64 " microseconds", span);
