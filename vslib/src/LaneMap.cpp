...
uint32_t LaneMap::getLinecardIndex() const
{
    SWSS_LOG_ENTER();
        ...
        _In_ const std::string& ifname)
{
    ...
    {
        SWSS_LOG_ERROR("interfce %s not found in laneMap %u", ifname.c_str(), m_linecardIndex);
        return false;
    }
        ...
        _In_ uint32_t linecardIndex)
{
    ...
    auto lm = std::make_shared<LaneMap>(linecardIndex);
        ...
        _In_ uint32_t laneNumber) const
{
    ...
    {
        SWSS_LOG_WARN("lane %u not found on index %u", laneNumber, m_linecardIndex);
