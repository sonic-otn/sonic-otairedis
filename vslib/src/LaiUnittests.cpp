...
void Lai::startUnittestThread()
{
    ...
    m_unittestChannelNotificationConsumer = std::make_shared<swss::NotificationConsumer>(m_dbNtf.get(), LAI_VS_UNITTEST_CHANNEL);

    m_unittestChannelRun = true;

    m_unittestChannelThread = std::make_shared<std::thread>(std::thread(&Lai::unittestChannelThreadProc, this));
}

void Lai::stopUnittestThread()
{
    SWSS_LOG_ENTER();
...
void Lai::channelOpEnableUnittest(
...
void Lai::channelOpSetReadOnlyAttribute(
        _In_ const std::string &key,
        _In_ const std::vector<swss::FieldValueTuple> &values)
{
    SWSS_LOG_ENTER();
    ...
    lai_deserialize_object_type(str_object_type, object_type);

    if (object_type == LAI_OBJECT_TYPE_NULL || object_type >= LAI_OBJECT_TYPE_EXTENSIONS_MAX)
    {
    ...
    lai_deserialize_object_id(str_object_id, object_id);

    lai_object_type_t ot = objectTypeQuery(object_id);

    if (ot != object_type)
    {
        SWSS_LOG_ERROR("object type is different than provided %s, but oid is %s",
                str_object_type.c_str(), lai_serialize_object_type(ot).c_str());
        return;
    }
    ...
    auto meta = lai_metadata_get_attr_metadata_by_attr_id_name(str_attr_id.c_str());
    ...
    if (meta->objecttype != ot)
    {
        SWSS_LOG_ERROR("attr %s belongs to different object type than oid: %s",
                str_attr_id.c_str(), lai_serialize_object_type(ot).c_str());
        return;
    }
    ...
    lai_deserialize_attr_value(str_attr_value, *meta, attr);

    SWSS_LOG_NOTICE("linecard id is %s", lai_serialize_object_id(linecard_id).c_str());

    lai_status_t status = m_meta->meta_unittests_allow_readonly_set_once(meta->objecttype, meta->attrid);

    if (status != LAI_STATUS_SUCCESS)
    {
        SWSS_LOG_ERROR("failed to enable SET readonly attribute once: %s", lai_serialize_status(status).c_str());
        return;
    }

    lai_object_meta_key_t meta_key = { .objecttype = ot, .objectkey = { .key = { .object_id = object_id } } };
    ...
    lai_deserialize_free_attribute_value(meta->attrvaluetype, attr);
}

void Lai::channelOpSetStats(
        _In_ const std::string &key,
        _In_ const std::vector<swss::FieldValueTuple> &values)
{
    SWSS_LOG_ENTER();
    ...
    lai_deserialize_object_id(key, oid);
    ...
    lai_object_id_t linecard_id = linecardIdQuery(oid);
    ...
    auto statenum = lai_metadata_get_object_type_info(ot)->statenum;
    ...
    std::map<lai_stat_id_t, uint64_t> stats;
...
void Lai::handleUnittestChannelOp(
        ...
        _In_ const std::vector<swss::FieldValueTuple> &values)
{
    ...
    if (op == LAI_VS_UNITTEST_ENABLE_UNITTESTS)
    {
        channelOpEnableUnittest(key, values);
    }
    else if (op == LAI_VS_UNITTEST_SET_RO_OP)
    {
        channelOpSetReadOnlyAttribute(key, values);
    }
    else if (op == LAI_VS_UNITTEST_SET_STATS_OP)
    {
...
void Lai::unittestChannelThreadProc()
{
    SWSS_LOG_ENTER();
