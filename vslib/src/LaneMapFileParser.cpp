        ...
        _In_ const std::string& slanes)
{
    ...
    auto lm = container->getLaneMap(linecardIndex);

    if (!lm)
    {
        lm = std::make_shared<LaneMap>(linecardIndex);
        ...
        _In_ const std::vector<std::string>& tokens)
{
    ...
    parse(container, LaneMap::DEFAULT_LINECARD_INDEX, ifname, slanes);
}
        ...
        _In_ const std::vector<std::string>& tokens)
{
    ...
    parse(container, linecardIndex, ifname, slanes);
}
