        ...
        _In_ const std::string& scpidx)
{
    ...
    auto corePortIndexMap = container->getCorePortIndexMap(linecardIndex);

    if (!corePortIndexMap)
    {
        corePortIndexMap = std::make_shared<CorePortIndexMap>(linecardIndex);
        ...
        _In_ const std::vector<std::string>& tokens)
{
    ...
    parse(container, CorePortIndexMap::DEFAULT_LINECARD_INDEX, ifname, scpidx);
}
        ...
        _In_ const std::vector<std::string>& tokens)
{
    ...
    parse(container, linecardIndex, ifname, scpidx);
}
